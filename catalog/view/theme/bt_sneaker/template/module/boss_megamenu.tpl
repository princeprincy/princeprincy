		...
		<?php if($status){ ?><!-- Mega menu -->
			...
			<div class="cs_mega_menu hidden-xs" >
				...
				<li class="menu_item level-1 parent dropdown">
					<a class="title_menu_parent<?php if (count($menu['menuchilds']) > 0) { echo ' sub_child'; }?> dropdown-toggle" href="<?php echo $menu['href']; ?>">
						<span class="menu-title"><?php echo $menu['title'] ?><i class="fa fa-caret-down"></i></span>
						<?php if (count($menu['menuchilds']) > 0) { echo ' <i class="fa fa-caret-down"></i>'; }?>
					</a>
