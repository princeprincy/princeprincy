				...
				<div id="opt_product">
					...
					<div class="scrollbox" id="product_list">
						...
						<?php if($ids[0]!='') { ?>
							...
							<?php if(isset($products_name[$product_id])){ ?>
								<?php $divclass = ($divclass == 'even' ? 'odd' : 'even'); ?>
							  <div class="<?php echo $divclass; ?>" id="product-item-<?php echo $product_id; ?>"><?php echo $products_name[$product_id]; ?><img src="view/image/delete.png" /><input name="bossmegamenu[product_ids][]" type="hidden" value="<?php echo $product_id; ?>" /></div>
							<?php } ?>
							<?php } ?>
						<?php } ?>
					</div>
